...
class diffeomorphicTransformer3D(Layer):
    ...
    def call(self, xx, mask=None):
        ...
        try:
            ...
            # TODO : ask Maria about bare except
            pass
        ...
        base_grid = tf.reshape(output, (samples, x_dim, y_dim, z_dim, channels))

        return [K.cast(output, K.floatx()), defgrid - base_grid]


class diffeomorphicTransformer2D(Layer):
    def __init__(self, mode=0, **kwargs):
        self.mode = mode
        super(diffeomorphicTransformer2D, self).__init__(**kwargs)

    def _integralImage(self, x):
        x_s = K.cumsum(x[..., 0] * 2, axis=2)
        y_s = K.cumsum(x[..., 1] * 2, axis=1)
    ...
    def call(self, inp, mask=None):
        ...
        defgrid = self._integralImage(defgrad)
        ...
        try:
            ...
            defgrid = tf.concat((defgrid, tf.ones((samples, height, width, 1))), -1)
            defgrid = tf.matmul(tf.reshape(defgrid, (samples, -1, 3)), affine, transpose_b=True)
            defgrid = tf.reshape(defgrid, (samples, height, width, 3))
        except:
            pass

        x_s, y_s = defgrid[:, :, :, 0], defgrid[:, :, :, 1]
        ...
        base_grid = tf.reshape(output, (samples, height, width, channels))
